"""Fig 2a replica for Qwen2.5-7B-Instruct LoRA sweep.
Baselines pulled from pokec_dataset/results/ (model-agnostic).
LLM 7B runs pulled from /Users/gabesmithline/Desktop/results_7b_sweep_1/.
"""
import os
import pickle
import numpy as np
...
LLM_RES = "/Users/gabesmithline/Desktop/results_7b_sweep_1/"
OUT = "figs"
...
runs = [
    ...
    ("llm_llm_7b_sft",              LLM_RES,      "LLM SFT (b=0)",     "#1f77b4", "-"),
    ("llm_llm_7b_sftkl_b0p3",       LLM_RES,      "LLM SFT+KL b=0.3",  "#2ca02c", "-"),
    ("llm_llm_7b_sftkl_b1",         LLM_RES,      "LLM SFT+KL b=1",    "#ff7f0e", "-"),
    ("llm_llm_7b_sftkl_b3",         LLM_RES,      "LLM SFT+KL b=3",    "#d62728", "-"),
    ("llm_llm_7b_sftkl_b10",        LLM_RES,      "LLM SFT+KL b=10",   "#9467bd", "-"),
]
...
axes[0].set_title("mean opinion across rounds (Qwen2.5-7B, LoRA SFT)")
...
axes[1].set_title("variance across rounds (Qwen2.5-7B, LoRA SFT)")
axes[1].grid(alpha=0.3)
axes[1].legend(fontsize=8, loc="best", ncol=2)

fig.suptitle("Fig 2a (7B replica): opinion after peer interaction, mean and variance")
fig.tight_layout()
out_path = os.path.join(OUT, "fig2a_mean_variance_7b.png")
fig.savefig(out_path, dpi=130)
print(f"wrote {out_path}")

print("\nFinal round population stats:")
print(f"  {'method':24s}  mean     dinnate     var")
