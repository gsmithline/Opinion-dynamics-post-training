    ...
    for ax, beta in zip(axes, args.betas):
        ...
        if args.include_track:
            ...
            if traj_tl is not None:
                mu_tl = _mu_vbackslash_s(traj_tl, mask_low, n_lab)
        ...
        ax.set_xlabel("round")
        ax.set_ylabel("mu_V\\S(t) - mu_V\\S(0)")
        ax.grid(alpha=0.3)

    axes[0].legend(loc="best", fontsize=8)
    fig.suptitle(f"Pull-reversal test on V\\S (unlabeled), feature = {args.feature}",
                 y=1.02)
    fig.tight_layout()
